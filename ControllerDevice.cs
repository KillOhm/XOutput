    ...
    public class ControllerDevice
    {
        ...
        byte Analog(byte subType, byte num)
        {
            ...
            switch (subType)
            {
                ...
                case 3: //Inverted Half
                    ...
                    return (byte)-m;
                case 4: //Fixed Default Value   // KillOhm - added to prevent unmapped axes from being stuck in an undesirable position
                    m=0; //-100%
                    if (num==1) m=128; //0%
                    if (num==2) m=255; //100%
                    return (byte)m;
        ...
        private void updateInput()
        {
            ...
            for (int i = 0; i < 21; i++)
            {
                ...
                output[i] = funcArray[type](subtype, num);  //Associates Xoutput with actual joystick values via mapping?
            }
